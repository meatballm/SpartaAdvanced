...
public class Deposit : DealMoney
{
    ...
    private void Start()
    {
        ...
        foreach (Button btn in buttons)
        {
            if (btn.CompareTag(targetTag))
            {
                ...
                capturedBtn.onClick.AddListener(() => OnDeposit(capturedBtn));
            }
            if (btn.CompareTag(inputTag))
            {
                btn.onClick.AddListener(() => OnDepositInput());
            }
        }
    }
    public void OnDeposit(Button btn)
    {
        Text text = btn.GetComponentInChildren<Text>();
        if (decimal.TryParse(text.text, NumberStyles.AllowThousands, CultureInfo.InvariantCulture, out amount))
        {
            MoneyMoved(true, amount);
